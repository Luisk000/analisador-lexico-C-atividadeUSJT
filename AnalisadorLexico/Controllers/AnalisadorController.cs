...
namespace AalisadorLexico.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class AnalisadorController : Controller
    {
        ...
        [HttpPost]
        [Route("[action]")]
        public IActionResult Analisar([FromBody]string entrada)
        {
            return Json(Result(entrada));
        }
        ...
        private string Result(string txt, string tt = @"matrix.txt")
        {         
            try
            {
